...
export function AdminLayout({ children }: AdminLayoutProps) {
  ...
  return (
    <div className="flex min-h-screen bg-[#0a1628]">
      ...
      {/* Sidebar */}
      <aside className={cn(
        "fixed inset-y-0 left-0 z-50 w-64 transform border-r border-white/10 bg-[#050d1a] transition-transform duration-200 ease-in-out lg:static lg:translate-x-0",
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex h-16 items-center justify-between border-b border-white/10 px-4">
            ...
            <Button 
              variant="ghost" 
              size="icon" 
              className="lg:hidden text-gray-400 hover:bg-white/10 hover:text-white"
              onClick={() => setSidebarOpen(false)}
            >
              <X className="h-5 w-5" />
            </Button>
          ...
          <nav className="flex-1 px-3 py-6">
            ...
            <div className="mb-2 mt-8 px-3 text-xs font-medium uppercase tracking-wider text-gray-500">
              Quick Links
            </div>
            <ul className="space-y-1">
              <li>
      ...
      <div className="flex flex-1 flex-col">
        {/* Mobile header */}
        <header className="flex h-16 items-center gap-4 border-b border-white/10 bg-[#0a1628] px-4 lg:hidden">
          <Button
            variant="ghost"
            size="icon"
