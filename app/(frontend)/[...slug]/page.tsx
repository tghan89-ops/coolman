...
/** Drop any block whose type isn't a registered component (malformed-block safety). */
function sanitize(data: PuckData): PuckData {
  const known = new Set(Object.keys(puckConfig.components))
  const content = Array.isArray(data?.content) ? data.content.filter((i) => i && known.has(i.type)) : []
